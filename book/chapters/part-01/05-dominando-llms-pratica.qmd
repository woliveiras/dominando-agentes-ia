# Capítulo 5: Dominando LLMs na Prática {.unnumbered}
...
O parâmetro **Temperature** controla quão "previsível" ou "criativo" o modelo será ao gerar texto [@fan2018hierarchical]. Pense nisso como um controle de volume entre "jogar sempre pelo seguro" e "se arriscar mais".
...
**O que é**: Em vez de usar um número fixo de tokens (top-k), nucleus sampling [@holtzman2019nucleus] considera os tokens cuja **probabilidade acumulada** atinge `p`.
...
Exemplos de context windows em modelos populares [@openai-api-reference; @anthropic-api-reference]:
...
::: {.callout-note title="Técnicas Avançadas de Extensão de Context Window"}
...
- **ALiBi (Attention with Linear Biases)** [@press2021train]: Permite extrapolação de comprimento em tempo de inferência
- **FlashAttention** [@dao2022flashattention]: Otimização de memória que torna atenção em contextos longos viável
- **Sparse Attention** [@beltagy2020longformer]: Padrões de atenção esparsos para documentos longos
- **Grouped-Query Attention** [@ainslie2023gqa]: Reduz footprint de memória do KV-cache
- **PagedAttention** [@kwon2023efficient]: Gerenciamento eficiente de memória para serving de LLMs (vLLM)
...
A maioria dos provedores de LLM cobra por tokens processados [@openai-tokenizer], separando **input tokens** (o que você envia) e **output tokens** (o que o modelo gera). Output tokens geralmente custam mais, pois exigem mais processamento.
...
3. **Generation latency**: Tempo para gerar cada token após o primeiro
   ...
   - *Fonte*: Observações em produção e [@artificialanalysis2024]
...
Para sistemas críticos, implemente load balancing entre múltiplos providers e fallbacks automáticos. Isso aumenta a resiliência, otimiza custos e melhora a latência.

O exemplo abaixo mostra uma implementação mais concreta de um `LLMLoadBalancer` que pode usar provedores reais (como OpenAI e Anthropic) ou mocks para demonstração.
...
class LLMProvider(Protocol):
    """Interface que todos os provedores de LLM devem seguir."""
    name: str
    async def generate(self, prompt: str, timeout: float) -> str: ...

@dataclass
class ProviderConfig:
    """Configuração para um provedor no load balancer."""
    provider: LLMProvider
    priority: int  # 0 = mais alta
    timeout_seconds: float = 10.0

class LoadBalancingStrategy(Enum):
    """Estratégias de balanceamento de carga."""
    PRIORITY = "priority"  # Sempre tenta o de maior prioridade primeiro
    ROUND_ROBIN = "round_robin" # Alterna entre os provedores

# --- Implementações Concretas de Provedores ---
# (Requerem API keys configuradas como variáveis de ambiente)
...
class OpenAIProvider:
    ...
    def __init__(self, model: str = "gpt-4-turbo"):
        self.model = model
        # O cliente async da OpenAI lê a key de OPENAI_API_KEY
...
class AnthropicProvider:
    """Implementação concreta para a API da Anthropic."""
    name = "anthropic"
    def __init__(self, model: str = "claude-3-5-sonnet-20240620"):
        self.model = model
        # O cliente async da Anthropic lê a key de ANTHROPIC_API_KEY
...
@dataclass
class ProviderStats:
    """Estatísticas de uso para um provedor."""
    calls: int = 0
    failures: int = 0
...
class LLMLoadBalancer:
    """
    Load balancer inteligente para múltiplos provedores de LLM.
    Oferece fallback automático e distribuição de carga.
    """
    def __init__(
        self,
        ...
        self.stats: Dict[str, ProviderStats] = {
            p.provider.name: ProviderStats() for p in self.providers
        }

    async def generate(self, prompt: str) -> Dict[str, Any]:
        """Gera uma resposta com fallback automático entre provedores."""
        last_error = None
        provider_order = self._get_provider_order()

        for attempt, config in enumerate(provider_order):
            ...
            try:
                print(f"Tentativa {attempt + 1}: Usando {provider.name}...")
                response = await asyncio.wait_for(
                    provider.generate(prompt, timeout=config.timeout_seconds),
                    timeout=config.timeout_seconds + 0.5
                )
                latency = time.time() - start_time
                stats.total_latency += latency
                print(f"✓ Sucesso com {provider.name} em {latency:.2f}s")
                return {
                    "response": response,
                ...
                stats.failures += 1
                last_error = e
                print(f"✗ Falha com {provider.name}: {e.__class__.__name__}")
                await asyncio.sleep(0.1)

        raise Exception(f"Todos os provedores falharam. Último erro: {last_error}")

    def _get_provider_order(self) -> List[ProviderConfig]:
        if self.strategy == LoadBalancingStrategy.ROUND_ROBIN:
            # Rotaciona a lista de provedores
            order = self.providers[self.current_index:] + self.providers[:self.current_index]
            self.current_index = (self.current_index + 1) % len(self.providers)
            return order
        return self.providers # Estratégia PRIORITY

    def print_stats(self):
        """Imprime as estatísticas de uso dos provedores."""
        print("\n--- Estatísticas Finais ---")
        for name, s in self.stats.items():
            print(
                f"- {name}: "
                f"Chamadas={s.calls}, Falhas={s.failures}, "
                f"Sucesso={s.success_rate:.1%}, "
                f"Latência Média={s.avg_latency * 1000:.0f}ms"
            )

# --- Exemplo de Uso ---
async def main():
    # Para usar provedores reais, descomente e configure suas API keys
    # Ex: export OPENAI_API_KEY="..."
    # Ex: export ANTHROPIC_API_KEY="..."
    
    # Usaremos mocks para a demonstração ser executável por todos.
    class MockProvider:
        def __init__(self, name, min_delay, max_delay, fail_rate):
            self.name = name
        ...
        async def generate(self, prompt: str, timeout: float) -> str:
            ...
            if delay > timeout: raise asyncio.TimeoutError(f"{self.name} timeout")
            if random.random() < self._fail_rate: raise ConnectionError(f"{self.name} API error")
            await asyncio.sleep(delay)
            return f"Resposta de {self.name}: {prompt[:20]}..."

    providers = [
        ProviderConfig(provider=MockProvider("OpenAI", 0.5, 1.5, 0.1), priority=0, timeout_seconds=1.0),
        ProviderConfig(provider=MockProvider("Anthropic", 1.0, 2.5, 0.05), priority=1, timeout_seconds=2.0),
        ProviderConfig(provider=MockProvider("Google", 1.5, 3.0, 0.02), priority=2, timeout_seconds=3.0),
    ]

    balancer = LLMLoadBalancer(providers, strategy=LoadBalancingStrategy.PRIORITY)

    print("--- Executando 10 chamadas de teste ---")
    for i in range(10):
        print(f"\n--- Chamada {i+1} ---")
        try:
            result = await balancer.generate(f"Prompt de teste {i+1}")
...
if __name__ == "__main__":
    # Para rodar: salve como um arquivo .py e execute `python nome_do_arquivo.py`
    # Lembre-se de instalar as dependências: uv pip install openai anthropic
    asyncio.run(main())
```
...
Sistemas de LLM em produção exigem observabilidade robusta [@prometheus-docs; @opentelemetry-docs] para garantir performance, controlar custos e diagnosticar problemas rapidamente. Diferente de aplicações tradicionais, LLMs introduzem métricas específicas (tokens, custos variáveis, qualidade de resposta) que precisam de instrumentação dedicada.
...
Uma stack completa de observabilidade para sistemas de LLM [@sigelman2010dapper; @opentelemetry-docs] consiste em três pilares:
...
1. **Métricas** [@prometheus-docs]: Agregações numéricas (latência P95, custo/hora, tokens/segundo)
2. **Traces** [@opentelemetry-docs]: Rastreamento distribuído de chamadas LLM e dependências
...
**Dashboard Grafana Recomendado** [@grafana-docs]:
...
Para consolidar seu entendimento através de implementação prática, consulte os [Exercícios Práticos do Capítulo 5](exercises/05-exercises.qmd), que incluem:

1. **Exploração de Parâmetros de Geração**: Experimente empiricamente temperature, top-p e top-k
2. **Gerenciamento de Context Window**: Implemente estratégias de sliding window, summarization e híbrida
3. **Otimização de Custo e Latência**: Construa sistema de caching e model routing
4. **Observabilidade e Monitoramento**: Instrumente aplicação com Prometheus e OpenTelemetry
...
4. **Observabilidade em Produção**: Instrumentação completa usando padrões da indústria:
   - Métricas com Prometheus [@prometheus-docs] (latência P95/P99, throughput, custos)
   - Traces distribuídos com OpenTelemetry [@opentelemetry-docs]
   - Logs estruturados com structlog
   - Alertas inteligentes (SLOs, budgets)

::: {.callout-tip title="Leituras Complementares Recomendadas"}
Para aprofundamento nos tópicos deste capítulo:

- **Sampling Strategies**: [@holtzman2019nucleus] para entender nucleus sampling e problemas de degeneração
- **Context Window Extensions**: [@press2021train; @dao2022flashattention] sobre técnicas modernas de extensão
- **Observability**: [@sigelman2010dapper] paper fundacional do Google sobre tracing distribuído
- **Performance Benchmarks**: [@artificialanalysis2024] para comparações atualizadas de modelos
- **API Documentation**: [@openai-api-reference; @anthropic-api-reference] documentação oficial atualizada
:::
