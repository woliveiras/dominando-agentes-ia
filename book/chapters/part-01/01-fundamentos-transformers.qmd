...
print(tokens)  # ['intel', 'ig', 'Ãª', 'nc', 'ia'] - 5 tokens
...
Para um exemplo detalhado que compara GPT-2, BERT e XLM-RoBERTa, demonstra decodificação e analisa eficiência entre idiomas, veja o @sec-exemplo-tokenizacao na seção de Exemplos de Código Completos.
:::
...
Para a implementação completa com documentação detalhada de cada passo, veja o @sec-exemplo-self-attention.
:::
...
%%| fig-cap: "Visualização do Multi-Head Attention: a entrada é dividida, processada por múltiplos heads de atenção em paralelo, e os resultados são concatenados e projetados."
%%| fig-width: 100%
graph TD
    A["Input (Q, K, V)"] --> H1["Head 1:<br/>Linear Projections<br/>W_q1, W_k1, W_v1"]
    A --> H2["Head 2:<br/>Linear Projections<br/>W_q2, W_k2, W_v2"]
    A --> H3["Head 3:<br/>Linear Projections<br/>W_q3, W_k3, W_v3"]
    A --> HD["..."]
    A --> Hn["Head h:<br/>Linear Projections<br/>W_qh, W_kh, W_vh"]
    
    H1 --> S1["Scaled Dot-Product<br/>Attention 1"]
    H2 --> S2["Scaled Dot-Product<br/>Attention 2"]
    H3 --> S3["Scaled Dot-Product<br/>Attention 3"]
    Hn --> Sn["Scaled Dot-Product<br/>Attention h"]
...
::: {.callout-note}
# Implementação e Exemplo de Uso Completos
Para a implementação completa com documentação e exemplo de uso em geração autoregressiva, veja o @sec-exemplo-kv-cache.
:::
...
ALiBi [@press2021train] não adiciona embeddings posicionais às entradas. Em vez disso, adiciona um viés aos attention scores proporcionalmente à distância entre tokens:
...
::: {.callout-note}
# Implementação Completa
Para a implementação completa com documentação detalhada de cada passo do roteamento, veja o @sec-exemplo-moe.
:::
...
::: {.callout-note}
# Implementação Completa
Para a implementação completa com documentação detalhada e visualizações, veja o @sec-exemplo-causal-mask.
:::
...
### Exemplo 1: Experimentando Tokenização com Diferentes Modelos {#sec-exemplo-tokenizacao}
...
### Exemplo 3: KV-Cache para Geração Autoregressiva {#sec-exemplo-kv-cache}
...
### Exemplo 4: Máscara Causal para Modelos Decoder-Only {#sec-exemplo-causal-mask}
...
### Exemplo 5: Mixture of Experts (MoE) Layer {#sec-exemplo-moe}
