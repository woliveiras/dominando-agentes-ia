...
Os foundation models representam uma mudança de paradigma em como construímos sistemas de IA. Antes deles, cada aplicação de machine learning seguia o mesmo padrão custoso:

1. **Definir uma tarefa específica** (ex: classificação de sentimentos, tradução, reconhecimento de entidades)
2. **Coletar e anotar milhares de exemplos** específicos para essa tarefa
3. **Treinar um modelo do zero** ou partir de um modelo pré-treinado genérico
4. **Repetir todo o processo** para cada nova tarefa ou domínio

Imagine construir um agente de atendimento ao cliente. Nessa abordagem antiga, você precisaria:

- Um modelo para entender intenção do usuário
- Outro modelo para extrair informações (nome, número de pedido, etc.)
- Outro para classificar o tipo de problema
- Outro para gerar respostas
- E mais modelos para cada nova funcionalidade

Cada um desses modelos exigia seu próprio dataset anotado, seu próprio processo de treinamento, e sua própria infraestrutura de deployment. O custo e complexidade eram proibitivos para a maioria das organizações.

**Foundation models invertem essa equação.** Em vez de treinar modelos especializados para cada tarefa, treinamos um único modelo massivo em dados diversos (texto, código, imagens) que desenvolve capacidades gerais de compreensão e geração. Esse modelo pode então ser **adaptado** para tarefas específicas através de **Prompting**, simplesmente descrever a tarefa em linguagem natural, **Few-shot learning**, fornecer alguns exemplos da tarefa, **Fine-Tuning**, ajustar o modelo com dados específicos do domínio (tema do Capítulo 3)

O termo "foundation" (fundação) é propositalmente arquitetônico, esses modelos servem como **base sobre a qual construímos aplicações especializadas**, da mesma forma que fundações de edifícios suportam estruturas diversas acima delas.

A hipótese central do porque isso funciona é que linguagem (e outras modalidades) contêm padrões profundos e transferíveis. Um modelo que aprende a prever a próxima palavra em textos diversos desenvolve, como efeito colateral, representações internas ricas sobre:

- Estrutura gramatical e semântica
- Conhecimento factual sobre o mundo
- Raciocínio lógico e matemático
- Padrões de código e algoritmos
- Até mesmo teoria da mente (entender intenções e crenças)

Essas representações podem ser aproveitadas para tarefas que o modelo nunca viu explicitamente durante o treinamento, uma propriedade chamada **capacidade emergente** que exploraremos mais adiante.

Formalmente, o termo "foundation model" foi cunhado por pesquisadores de Stanford em 2021 [@bommasani2021opportunities] para descrever modelos que são: (1) treinados em dados amplos e diversos usando *self-supervision*, (2) servem como base para uma variedade de tarefas downstream, e (3) demonstram capacidades emergentes que não foram explicitamente programadas. Large Language Models (LLMs) como GPT [@brown2020language], BERT [@devlin2018bert], LLaMA [@touvron2023llama] e Claude são exemplos proeminentes de foundation models, mas o conceito se estende a outras modalidades como CLIP e DALL-E para imagens ou Codex e StarCoder para código.
...
Este exemplo prático demonstra como a tokenização funciona "nos bastidores" dos foundation models que você usará para construir agentes.
