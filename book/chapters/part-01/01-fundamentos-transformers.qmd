...
Tokens são as **unidades básicas de processamento** que os modelos de linguagem utilizam. Pense neles como os "pedaços" ou "blocos" em que o texto é dividido antes de ser processado pelo modelo. Diferente do que você pode imaginar inicialmente, tokens não são necessariamente palavras completas.

### Por Que Não Usar Palavras Diretamente?

Uma pergunta natural é: por que não simplesmente usar palavras como unidade básica? A resposta está na praticidade e eficiência:

1. **Vocabulário imenso**: O português possui centenas de milhares de palavras. Considerar todas as flexões verbais, conjugações, e variações seria inviável. Imagine para outros idiomas com ainda mais complexidade morfológica!
2. **Palavras novas**: Novas palavras surgem constantemente (neologismos, gírias, termos técnicos, nomes próprios).
3. **Multilinguismo**: Um modelo que precisa trabalhar com múltiplos idiomas teria um vocabulário astronômico.
4. **Eficiência computacional**: Um vocabulário menor significa menos parâmetros no modelo e processamento mais rápido.

### Como Funciona a Tokenização?

Os modelos modernos usam algoritmos sofisticados de **sub-word tokenization** como **Byte-Pair Encoding (BPE)**, **WordPiece** ou **SentencePiece**. Esses algoritmos funcionam da seguinte forma:

1. **Começam com caracteres**: Inicialmente, cada caractere é um token
2. **Identificam padrões frequentes**: Encontram sequências de caracteres ou sub-palavras que aparecem frequentemente juntas
3. **Mesclam em tokens**: Combinam esses padrões frequentes em tokens únicos
4. **Repetem o processo**: Continuam mesclando até atingir um tamanho de vocabulário desejado (geralmente entre 30.000 e 100.000 tokens)
