...
Um dos avanços fundamentais dos Transformers em relação a word embeddings estáticos (word2vec, GloVe) é que eles produzem **representações contextualizadas**. A mesma palavra recebe representações diferentes dependendo do contexto:

```
"The bank of the river" -> bank₁
"The bank is closed" -> bank₂
```

Estas representações capturam:

1. **Informação Sintática**: Parte do discurso, relações de dependência sintática
2. **Informação Semântica**: Significado, relações semânticas com outras palavras
3. **Informação Contextual**: Significado derivado do contexto específico
4. **Conhecimento Factual**: Propriedades e fatos sobre a entidade referenciada

Pesquisas em interpretabilidade têm mostrado que diferentes camadas de um Transformer capturam diferentes tipos de informação:

- **Camadas Iniciais**: Informação mais sintática e superficial
- **Camadas Intermediárias**: Informação semântica e factual  
- **Camadas Finais**: Informação mais task-specific e abstrata

Esta estrutura hierárquica sugere que LLMs aprendem representações progressivamente mais abstratas, similar a como CNNs aprendem features visuais progressivamente mais complexas.

### Capacidades Emergentes: Além do Treinamento

Um dos fenômenos mais surpreendentes dos LLMs é a emergência de capacidades que não foram explicitamente treinadas. Modelos suficientemente grandes demonstram:

- **In-Context Learning**: Aprender novas tarefas a partir de poucos exemplos no prompt, sem atualizar pesos [@brown2020language]
- **Chain-of-Thought Reasoning**: Resolver problemas complexos através de raciocínio passo-a-passo [@wei2022chain]
- **Few-Shot Learning**: Generalizar para novas tarefas com exemplos mínimos
- **Instruction Following**: Seguir instruções complexas expressas em linguagem natural
